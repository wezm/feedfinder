...
impl<'a> FeedFinder<'a> {
    fn meta_links(&self) -> FeedResult {
            ...
            .map_err(|_| FeedFinderError::Select)?
        {
            ...
            match (attrs.get("type"), attrs.get("href")) {
                (Some("application/rss+xml"), Some(href)) => feeds.push(Feed { url: self.base_url
                    .join(href)
                    .map_err(FeedFinderError::Url)?, type_: FeedType::Rss }),
                (Some("application/atom+xml"), Some(href)) => feeds.push(Feed { url: self.base_url
                    .join(href)
                    .map_err(FeedFinderError::Url)?, type_: FeedType::Atom }),
                (Some("application/json"), Some(href)) => feeds.push(Feed { url: self.base_url
                    .join(href)
                    .map_err(FeedFinderError::Url)?, type_: FeedType::Json }),
                _ => (),
            }
        }
    ...
    fn youtube(&self) -> FeedResult {
        ...
        if url.starts_with("https://www.youtube.com/channel/") {
            ...
            if let Some(id) = nth_path_segment(self.base_url, 1) {
                ...
                feeds.push(Feed { url: feed, type_: FeedType::Atom });
            }
        ...
        for a in self.doc.select("a").map_err(|_| FeedFinderError::Select)? {
            ...
            if let Some(href) = attrs.get("href") {
                if MIGHT_BE_FEED.iter().any(|hint| href.contains(hint)) {
                    feeds.push(Feed { url: self.base_url
                        .join(href)
                        .map_err(FeedFinderError::Url)?, type_: FeedType::Link })
                }
            }
        }
    ...
    fn guess(&self) -> FeedResult {
        ...
        let url = if markup.contains("tumblr.com") {
            ...
            Some(self.base_url.join("./index.xml").map_err(FeedFinderError::Url)?)
            ...
            || self.base_url
                ...
                .unwrap_or(false)
        {
            Some(self.base_url.join("./atom.xml").map_err(FeedFinderError::Url)?)
        ...
        Ok(url.map(|url| vec![Feed { url, type_: FeedType::Guess}]).unwrap_or_else(|| vec![]))
    }
}
...
fn test_detect_meta_atom() {
    ...
    assert_eq!(detect_feeds(&base, html), Ok(vec![Feed { url, type_: FeedType::Atom }]));
}
...
fn test_detect_meta_rss() {
    ...
    assert_eq!(detect_feeds(&base, html), Ok(vec![Feed { url, type_: FeedType::Rss }]));
}
...
fn test_detect_meta_json_feed() {
    ...
    assert_eq!(detect_feeds(&base, html), Ok(vec![Feed { url, type_: FeedType::Json }]));
}
...
fn test_body_link_feed() {
    ...
    assert_eq!(detect_feeds(&base, html), Ok(vec![Feed { url, type_: FeedType::Link }]));
}
...
fn test_guess_tumblr() {
    ...
    assert_eq!(detect_feeds(&base, html), Ok(vec![Feed { url, type_: FeedType::Guess }]));
}
